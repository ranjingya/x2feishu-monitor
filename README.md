...
对于超过普通长度限制的 X Note Tweet，程序会优先读取 `note_tweet.text` 完整正文，避免使用顶层 `text` 中被截断并附带 `t.co` 短链的兼容文本。
...
设置 `X_INCLUDE_REPLIES=true` 后，程序会在同一次用户帖子请求中读取回复及其上下文。回复卡片会展示被回复推文的作者、正文和链接，以及监控用户的回复、译文和回复原文链接。这个功能通过 X API 的 `referenced_tweets` 与 `includes` 扩展数据实现，不会为每条回复额外发起 X API 请求；只有新增内容超过单页容量时才会继续分页。

如果被回复推文已经删除、受保护或当前凭证无权访问，卡片仍会保留可获得的推文 ID 和链接，并将正文标记为不可用。
